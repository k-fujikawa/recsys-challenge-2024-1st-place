...
BASE_COLUMNS = list(
    itertools.chain(
        *[
            [
                f"{group_name}_scroll_percentage_filled_mean_past_all",
                f"{group_name}_scroll_percentage_filled_mean_future_all",
                f"{group_name}_scroll_percentage_filled_mean_past_and_future_all",
            ]
            + [
                f"{group_name}_scroll_percentage_filled_mean_past_{time_str}"
                for time_str in USE_TIMES + FIX_WINDOWS
            ]
            + [
                f"{group_name}_scroll_percentage_filled_mean_future_{time_str}"
                for time_str in USE_TIMES + FIX_WINDOWS
            ]
            + [
                f"{group_name}_scroll_percentage_filled_mean_past_and_future_{time_str}"
                for time_str in USE_TIMES + FIX_WINDOWS
            ]
...
USE_COLUMNS = [col + "_ratio" for col in BASE_COLUMNS]
...
def process_df(cfg, behaviors_df):
    ...
    for group_name, group_subset in [
        ...
        explode_df = explode_df.with_columns(
            [
                ...
                ).alias(f"{group_name}_scroll_percentage_filled_mean_past_all"),
                (
                    ...
                    .over(group_subset)
                ).alias(f"{group_name}_scroll_percentage_filled_mean_future_all"),
            ]
        )
        ...
        explode_df = explode_df.with_columns(
            [
                (
                    ...
                    .over(group_subset)
                ).alias(f"{group_name}_scroll_percentage_filled_mean_past_{time_str}")
        ...
        explode_df = explode_df.with_columns(
            [
                (
                    ...
                    .over(group_subset)
                ).alias(f"{group_name}_scroll_percentage_filled_mean_future_{time_str}")
                for time_str in USE_TIMES
            ]
            + [
                (
        ...
        explode_df = explode_df.with_columns(
            [
                (
                    pl.col(f"{group_name}_scroll_percentage_filled_mean_past_all")
                    + pl.col(f"{group_name}_scroll_percentage_filled_mean_future_all")
                ).alias(
                    f"{group_name}_scroll_percentage_filled_mean_past_and_future_all"
                ),
            ]
    ...
    for col in BASE_COLUMNS:
        explode_df = explode_df.with_columns(
            [
                (pl.col(col) - pl.col("scroll_percentage")).alias(f"{col}_ratio"),
            ]
        )
