...
def process_df(cfg, behaviors_df):
    ...
    behaviors_df = behaviors_df.filter(pl.col("impression_id") != 0)
    print(behaviors_df.shape)
    base_df = (
    ...
    model = AlternatingLeastSquares(
        ...
        num_threads=cfg.exp.num_threads,
    )
    model.fit(matrix, show_progress=True)
    user_df = pl.DataFrame(
        [pl.Series(name="user_id", values=user_id_list, dtype=base_df["user_id"].dtype)]
        + [
    ...
    article_df = pl.DataFrame(
        [
            pl.Series(
                ...
                dtype=base_df["article_id"].dtype,
            )
        ]
        + [
