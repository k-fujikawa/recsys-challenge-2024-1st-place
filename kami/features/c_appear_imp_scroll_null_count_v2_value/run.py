...
BASE_COLUMNS = list(
    itertools.chain(
        *[
            [
                f"{group_name}_scroll_null_count_mean_past_all",
                f"{group_name}_scroll_null_count_mean_future_all",
                f"{group_name}_scroll_null_count_mean_past_and_future_all",
            ]
            + [
                f"{group_name}_scroll_null_count_mean_past_{time_str}"
                for time_str in USE_TIMES + FIX_WINDOWS
            ]
            + [
                f"{group_name}_scroll_null_count_mean_future_{time_str}"
                for time_str in USE_TIMES + FIX_WINDOWS
            ]
            + [
                f"{group_name}_scroll_null_count_mean_past_and_future_{time_str}"
                for time_str in USE_TIMES + FIX_WINDOWS
            ]
...
def process_df(cfg, behaviors_df):
    behaviors_df = behaviors_df.select(
        ...
        [
            ...
            .is_null()
            .cast(pl.Int8)
            .alias("is_null_scroll_percentage"),
            (
                pl.when(pl.col("scroll_percentage").is_null())
                .then(0)
                .otherwise(pl.col("scroll_percentage"))
            ).alias("scroll_percentage_filled"),
        ]
    )
    explode_df = (
    ...
    for group_name, group_subset in [
        ...
        explode_df = explode_df.with_columns(
            [
                (
                    pl.col("scroll_percentage_filled").cum_sum().over(group_subset)
                    / pl.col("scroll_percentage_filled").cum_count().over(group_subset)
                ).alias(f"{group_name}_scroll_null_count_mean_past_all"),
                (
                    pl.col("scroll_percentage_filled")
                    .cum_sum(reverse=True)
                    .over(group_subset)
                    / pl.col("scroll_percentage_filled")
                    .cum_count(reverse=True)
                    .over(group_subset)
                ).alias(f"{group_name}_scroll_null_count_mean_future_all"),
            ]
        )
        ...
        explode_df = explode_df.with_columns(
            [
                (
                    pl.col("scroll_percentage_filled")
                    .rolling_mean(window_size=time_str, by="impression_time")
                    .over(group_subset)
                ).alias(f"{group_name}_scroll_null_count_mean_past_{time_str}")
                for time_str in USE_TIMES
            ]
            + [
                (
                    pl.col("scroll_percentage_filled")
                    .rolling_mean(window_size=time_str)
                    .over(group_subset)
                ).alias(f"{group_name}_scroll_null_count_mean_past_{time_str}")
        ...
        explode_df = explode_df.with_columns(
            [
                (
                    pl.col("scroll_percentage_filled")
                    .rolling_mean(window_size=time_str, by="reverse_impression_time")
                    .over(group_subset)
                ).alias(f"{group_name}_scroll_null_count_mean_future_{time_str}")
                for time_str in USE_TIMES
            ]
            + [
                (
        ...
        explode_df = explode_df.with_columns(
            [
                (
                    pl.col(f"{group_name}_scroll_null_count_mean_past_all")
                    + pl.col(f"{group_name}_scroll_null_count_mean_future_all")
                ).alias(f"{group_name}_scroll_null_count_mean_past_and_future_all"),
            ]
            + [
                (
                    pl.col(f"{group_name}_scroll_null_count_mean_past_{time_str}")
                    + pl.col(f"{group_name}_scroll_null_count_mean_future_{time_str}")
                ).alias(
                    f"{group_name}_scroll_null_count_mean_past_and_future_{time_str}"
                )
                for time_str in USE_TIMES + FIX_WINDOWS
