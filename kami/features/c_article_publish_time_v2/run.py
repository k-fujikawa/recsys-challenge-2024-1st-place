...
USE_COLUMNS = [
    "time_min_diff",
    "time_min_diff_rn",
    "time_min_diff_session_rn",
    "time_min_diff_user_rn",
    "time_hour_diff",
    "time_day_diff",
    "impression_weekday",
    "impression_hour",
...
def process_df(cfg, behaviors_df, articles_df):
    ...
    candidate_df = candidate_df.with_columns(
        [
            ...
            .dt.total_days()
            .alias("time_day_diff"),
            pl.col("impression_time").dt.weekday().alias("impression_weekday"),
            pl.col("impression_time").dt.hour().alias("impression_hour"),
            pl.col("published_time").dt.weekday().alias("published_weekday"),
            pl.col("published_time").dt.hour().alias("published_hour"),
        ]
    ).with_columns(
        [
            ...
            .alias("time_min_diff_session_rn"),
            pl.col("time_min_diff")
            .rank()
            .over(["user_id"])
            .alias("time_min_diff_user_rn"),
        ]
    )
