...
USE_COLUMNS = ["is_viewed", "is_last_viewed"]
...
def process_df(cfg, behaviors_df):
    ...
    tmp_df = (
        ...
        .with_columns(pl.col("viewed_articles").alias("last_viewed_articles"))
    )
    ...
    candidate_df = candidate_df.with_columns(
        [
            pl.col("article_id").is_in(pl.col("viewed_articles")).alias("is_viewed"),
            pl.col("article_id")
            .is_in(pl.col("last_viewed_articles"))
