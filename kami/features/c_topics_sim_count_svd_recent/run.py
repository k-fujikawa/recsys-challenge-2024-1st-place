...
USE_COLUMNS = [
    "topics_count_svd_5_sim",
    "topics_count_svd_5_rn",
    "topics_count_svd_10_sim",
    "topics_count_svd_10_rn",
]
TARGET_COL = "body"
n_components = 50
...
def process_df(cfg, article_embeddings, articles_df, history_df, candidate_df):
    ...
    for num_recent in [5, 10]:
        ...
        candidate_df = candidate_df.with_columns(
            pl.Series(name=f"topics_count_svd_{num_recent}_sim", values=similarity)
        ).with_columns(
            pl.col(f"topics_count_svd_{num_recent}_sim")
            .rank(descending=True)
            .over("user_rn")
            .alias(f"topics_count_svd_{num_recent}_rn")
        )

    return candidate_df.select(USE_COLUMNS)
