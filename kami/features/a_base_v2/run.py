...
USE_COLUMNS = [
    ...
    f"entity_groups_{col}"
    for col in ["PER", "null", "LOC", "ORG", "PROD", "EVENT", "MISC"]
]


def process_df(cfg, df):
    # entity_groups を multi-hot に変換
    multi_hot_df = df.explode("entity_groups").pivot(
        index="article_id",
        columns="entity_groups",
        values="entity_groups",
        aggregate_function="len",
    )
    multi_hot_df = multi_hot_df.rename(
        {
            col: f"entity_groups_{col}"
            for col in multi_hot_df.columns
            if col != "article_id"
        }
    )
    print(multi_hot_df)
    print(multi_hot_df.columns)

    df = df.join(multi_hot_df, on="article_id", how="left")
